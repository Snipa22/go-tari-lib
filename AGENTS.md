...
- **What this repo is:** Reusable helper functions layered on top of `go-tari-grpc-lib`. Utility/convenience code that doesn't belong in the GRPC wrapper layer itself but is shared across multiple `go-tari-*` consumers.
- **Module path:** `github.com/Snipa22/go-tari-lib`
- **Depends on:** `go-tari-grpc-lib` (GRPC wrapper), `core-go-lib` (shared utilities)
